...
class LocalMistral(BaseModel):
    def __init__(self, model_name="mistralai/Mixtral-7B-v0.1", config=None):
        super().__init__(model_name, config)
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.model = AutoModelForCausalLM.from_pretrained(model_name)
        self.accelerate = AccelerateWrapper(self.model, config)
        self.model = self.accelerate.get_model()
        self.logger.info(f"Loaded local Mistral model: {model_name}")
...
class APIMistral(BaseModel):
    def __init__(self, model_name="mistralai/Mixtral-7B-v0.1", config=None):
        ...
        self.logger.info(f"Initialized API Mistral model: {model_name}")
